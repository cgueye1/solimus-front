...
@Component({
  selector: 'app-prescription-payment',
  standalone: true,
...
export class PrescriptionPaymentComponent implements OnInit {
  loading = true;
  error = '';
  ...
  ngOnInit(): void {
    const presId = Number(this.route.snapshot.paramMap.get('presId'));
    ...
    if (!presId || !amount) {
      ...
      this.callTouchPay(
        ...
        '', //nom
        '', //telephone
        `https://wakana.online/pharma-delivery/api/prescriptions/${presId}/pay`
      );
