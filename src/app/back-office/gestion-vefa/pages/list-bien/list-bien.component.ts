...
@Component({
  ...
  standalone: true,
  imports: [CardBienHorizComponent, CommonModule, FormsModule],
  templateUrl: './list-bien.component.html',
  styleUrl: './list-bien.component.scss',
})
export class ListBienComponent implements OnInit {
  ...
  getMe() {
    ...
    this.userService.getDatas(endpoint).subscribe({
      next: (data) => {
        ...
        if(this.currentUser.profil!=="NOTAIRE" && this.currentUser.profil!=="RESERVATAIRE"){
  ...
  onCreate() {
    this.router.navigate(['/gestion-vente-vefa/create-bien'], {
      queryParams: { rental: this.rental },
    });
  }
  ...
  onView(data: any) {
    if (
      ...
      if (this.rental) {
        ...
        this.router.navigate(['/gestion-vente-vefa', data.id, 'detail-bien']);
      }
    } else {
      this.router.navigate([data.id, 'detail-lot'], {
        relativeTo: this.activatedRoute,
  ...
  loadMorePropreties() {
    ...
    switch (this.currentUser.profil) {
      ...
      case ProfilEnum.AGENCY:
        endpoint = this.rental
          ? `/realestate/rental/search-by-promoter${pageParams}`
          : `/realestate/search-by-agency${pageParams}`;
        break;
      default:
        endpoint = this.rental
