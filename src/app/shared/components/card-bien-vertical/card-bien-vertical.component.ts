...
if( !this.data.rental)
   this.modalService.open(PopupDetailBienComponent, {
    ...
    }).componentInstance.singleBien = this.data; // Pass an example bienId*/
  }
  ...
  openPdf() {
    if (this.data.notary  && this.data.notary.technicalSheet) {
   ...
   openPdf1() {
    if (this.data.notary.technicalSheet) {
      const url = `${this.IMG_URL}/${this.data.legalStatus}`;
      window.open(url, '_blank');
    }
  }
